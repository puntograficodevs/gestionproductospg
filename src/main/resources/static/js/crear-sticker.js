document.addEventListener('DOMContentLoaded', () => {
      ...
      // Inputs y checkboxes
      const adicionalCheckbox = document.getElementById('sticker-con-adicional-disenio');
      ...
      async function calcularPrecio() {
        const troqueladoSeleccionado = document.querySelector('input[name="tipoTroqueladoSticker.id"]:checked');
        const cantidadSeleccionada = document.querySelector('input[name="cantidadSticker.id"]:checked');
        const medidaSeleccionada = document.querySelector('input[name="medidaSticker.id"]:checked');
        let tipoTroqueladoStickerId = 0;
        let cantidadStickerId = 0;
        let medidaStickerId = 0;
        ...
        if (!troqueladoSeleccionado || !cantidadSeleccionada || !medidaSeleccionada) {
            ...
            cantidadStickerId = Number(cantidadSeleccionada.value);
            medidaStickerId = Number(medidaSeleccionada.value);
        }

        try {
            const response = await fetch(`/api/plantilla-sticker/precio?tipoTroqueladoStickerId=${tipoTroqueladoStickerId}&cantidadStickerId=${cantidadStickerId}&medidaStickerId=${medidaStickerId}`);

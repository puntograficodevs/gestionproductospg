document.addEventListener('DOMContentLoaded', () => {
      ...
      // Inputs y checkboxes
      const adicionalCheckbox = document.getElementById('sobre-con-adicional-disenio');
      ...
      const radiosMedida = document.querySelectorAll('input[name="medidaSobre.id"]');
      const radiosColor = document.querySelectorAll('input[name="tipoColorSobre.id"]');
      const radiosCantidad = document.querySelectorAll('input[name="cantidadSobre.id"]');
      ...
      async function calcularPrecio() {
        const medidaSeleccionada = document.querySelector('input[name="medidaSobre.id"]:checked');
        const colorSeleccionado = document.querySelector('input[name="tipoColorSobre.id"]:checked');
        const cantidadSeleccionada = document.querySelector('input[name="cantidadSobre.id"]:checked');
        let medidaSobreId = 0;
        let tipoColorSobreId = 0;
        let cantidadSobreId = 0;
        let precioProducto = 0;

        if (!medidaSeleccionada || !colorSeleccionado || !cantidadSeleccionada) {
            totalInput.value = 0;
            precioImpuestosInput.value = 0;
            restaInput.value = 0;
            return;
        } else {
            medidaSobreId = Number(medidaSeleccionada.value);
            tipoColorSobreId = Number(colorSeleccionado.value);
            cantidadSobreId = Number(cantidadSeleccionada.value);
        }

        try {
            const response = await fetch(`/api/plantilla-sobre/precio?medidaSobreId=${medidaSobreId}&tipoColorSobreId=${tipoColorSobreId}&cantidadSobreId=${cantidadSobreId}`);
